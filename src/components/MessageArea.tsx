...
// Define the SearchInfo type for type safety
type SearchInfo = {
    query: string;
    stages: string[];
    urls?: string[];
...
const MessageArea = ({ messages, showPdfOption }: MessageAreaProps) => {
            ...
            <div className="max-w-4xl mx-auto p-6">
                {messages.map((message) => (
                    <div key={message.id} className={`flex ${message.isUser ? 'justify-end' : 'justify-start'} mb-5`}>
                        <div className="flex flex-col max-w-md">
                            ...
                            {/* Message Content */}
                            <div
                                className={`whitespace-pre-wrap break-words rounded-lg py-3 px-5 text-sm md:text-base leading-relaxed ${
                                    message.isUser
                ...
                {showPdfOption && (
                    <div className="text-center mt-6">
                        <a
                        href={`${process.env.NEXT_PUBLIC_API_BASE_URL}/generate-pdf`} // change this if your endpoint is different
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow inline-block"
                        >
                        Download JD as PDF
                        </a>
                    </div>
